...
setcookie("last_login", date("Y-m-d h:i:sa", time()), time()+60*60*24*30);
?>
...
<body id="main_body" >
		...
		<div id="form_container">
			...
			<form id="form_1" class="appnitro"  method="post" action="?step=2">
				<div class="form_description">
					<h2>Wypełnij formularz <button onclick="location.href='3.php?step=10'" type="button">Wczytaj</button></h2>
					<span>Ostatnie logowanie: <?php echo ($last_login); ?></span></br>
					<span>Liczba wyświetleń strony: <?php echo ($_SESSION["visits"]); ?></span>
				</div>						
				<ul >
	...
	if (isset($_POST['form_id']) && $_GET['step'] == 3) {
		...
		<span><b>Klient: </b></span><?php echo ($_POST['name']." ".$_POST['surname']); $content .= $_POST['name']." ".$_POST['surname'].";"; ?></br>
		<span><b>Ilość osób: </b></span><?php echo ($_POST['element_8']); $content .= $_POST['element_8'].";"; ?></br>
		<span><b>Numer karty kredytowej: </b></span><?php echo ($_POST['creditcard']); $content .= $_POST['creditcard'].";";?></br>
		<span><b>Adres: </b></span>
		<?php echo ($_POST['element_4_1']." ".$_POST['element_4_2']." ".
			$_POST['element_4_3']." ".$_POST['element_4_4']." ".$_POST['element_4_5'].
			" ".$_POST['element_4_6']." ");
			$content .= $_POST['element_4_1']." ".$_POST['element_4_2']." ".
			$_POST['element_4_3']." ".$_POST['element_4_4']." ".$_POST['element_4_5'].
			" ".$_POST['element_4_6'].";";
			?></br>
			<span><b>Data przyjazdu: </b></span><?php echo ($_POST['date']." ".$_POST['element_6_1'].":".$_POST['element_6_2']); 
			$content .= $_POST['date']." ".$_POST['element_6_1'].":".$_POST['element_6_2'].";"?></br>
			<span><b>Email: </b></span><?php echo ($_POST['element_7']); $content .= $_POST['element_7'].";"?></br>
			<ul>
				<?php if($_POST['element_9_1']==1) { $content .= "1;";?>
					<li>łóżko dla dziecka</li>
				<?php } else {$content .= "0;";} ?>
				<?php if($_POST['element_9_2']==1) { $content .= "1;";?>
					<li>klimatyzacja</li>
				<?php } else {$content .= "0;";}?>
				<?php if($_POST['element_9_3']==1) { $content .= "1;";?>
					<li>popielniczka dla palacza</li>
				<?php } else {$content .= "0;";}?>
			</ul>
		<?php for ($i = 1; $i<=intval($_POST['element_8']); $i++) { 
		echo ("<span><b>Gość ".$i.": </b></span>".$_POST['name_'.$i]." ".$_POST['surname_'.$i]." ".$_POST['birthday_'.$i]."</br>");
		$content .= "Gość ".$i.": ".$_POST['name_'.$i]." ".$_POST['surname_'.$i]." ".$_POST['birthday_'.$i]." ";
		}
		$content .= ";";
		file_put_contents("3.csv", $content."\r\n", FILE_APPEND | LOCK_EX);
		?>
		<?php }
		?>
		<?php
		if (isset($_GET['step']) && $_GET['step'] == 10) {
		$lines = file("3.csv");
		foreach($lines as $line){
		    $sr = explode(";" , $line);
		?>
		<h2> Podsumowanie rezerwacji: </h2>
		<span><b>Klient: </b></span><?php echo ($sr[0]); ?></br>
		<span><b>Ilość osób: </b></span><?php echo ($sr[1]); ?></br>
		<span><b>Numer karty kredytowej: </b></span><?php echo ($sr[2]);?></br>
		<span><b>Adres: </b></span>
		<?php echo ($sr[3]);?></br>
		<span><b>Data przyjazdu: </b></span><?php echo ($sr[4]);?></br>
			<span><b>Email: </b></span><?php echo ($sr[5]);?></br>
			<ul>
				<?php if($sr[6]==1) { ?>
					<li>łóżko dla dziecka</li>
				<?php } ?>
